...
{
	for(int i=0;i<3;i++)
		if(complete(i*3+1)==tor and complete(i*3+2)==tor and complete(i*3+3)==tor)
			return true;
	for(int j=1;j<=3;j++)
		if(complete(j)==tor and complete(j+3)==tor and complete(j+6)==tor)
			return true;
	if(complete(3)==tor and complete(5)==tor and complete(7)==tor) return true;
	if(complete(1)==tor and complete(5)==tor and complete(9)==tor) return true;
	return false;
}
